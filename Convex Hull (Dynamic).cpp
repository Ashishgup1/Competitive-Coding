...
struct ConvexHullDynamic {
    ...
    struct Line {
        int a, b;     // y = ax + b
        double xLeft; // Stores the intersection wiith previous line in the
                      // convex hull. First line has -INF

        enum Type { line, maxQuery, minQuery } type;
        int val;

        explicit Line(int aa = 0, int bb = 0)
    ...
    void addLine(int a, int b) // Add ax + b in logN time
    {
        ...
        // If parallel liune is already in set, one of the lines becomes
        // irrelevant
