    ...
    matrix operator*(const matrix &o) const {
        ...
        for (int i = 0; i < SZ; i++) {
            for (int j = 0; j < SZ; j++) {
                ...
                for (int k = 0; k < SZ; k++) {
                    res.arr[i][j] =
                        add(res.arr[i][j], mult(arr[i][k], o.arr[k][j]));
                }
            }
        }
...
// Sample Problem 3 (Only Matrix Expo):
// https://codeforces.com/contest/551/problem/D Sample Solution 3:
// https://codeforces.com/contest/551/submission/48459284
